...
use crate::{
    ...
    sleep::{self, wait_for},
    ColumnDescription, DataType, Error, Sleep,
};
...
pub trait AsyncResultSetMetadata: AsStatementRef {
    ...
    async fn num_result_cols(&mut self, mut sleep: impl Sleep) -> Result<i16, Error> {
        let stmt = self.as_stmt_ref();
        let res = wait_for(|| stmt.num_result_cols(), &mut sleep).await;
        res.into_result(&stmt)
    }
    ...
    async fn column_is_unsigned(
        &mut self,
        column_number: u16,
        mut sleep: impl Sleep,
    ) -> Result<bool, Error> {
        let stmt = self.as_stmt_ref();
        wait_for(|| stmt.is_unsigned_column(column_number), &mut sleep)
            .await
            .into_result(&stmt)
    }
