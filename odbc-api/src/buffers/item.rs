...
impl_item!(Time, time, Time, NullableTime);
...
mod tests {
    use crate::{
        BindParamDesc,
        buffers::item::DEFAULT_TIME_PRECISION,
    ...
    fn item_to_bind_param_desc() {
        ...
        assert_eq!(BindParamDesc::time(false), Time::bind_param_desc(false));
        assert_eq!(
            BindParamDesc::timestamp(false, DEFAULT_TIME_PRECISION),
